...
impl Core {
    pub async fn handle_pre_commit_vote(&mut self, author: PublicKey, view: u64, signature: Signature, node: Node) -> ConsensusResult<()> {
        debug!("Received pre commit vote for view {:?}", view);
        if view < self.view {
            return Ok(());
        }
        ...
        if let Some(pre_commit_qc) = self.aggregator.add_pre_commit_vote(author, view, signature, node)? {
            debug!("Formed PreCommit QC for view {:?}", view);
            self.lock_qc = pre_commit_qc.clone();
        ...
        let commit_message = ConsensusMessage::new(
         ...
         match bincode::serialize(&commit_message) {
                Ok(payload) => {
                 ...
                 self.handle_commit(self.name, self.view, pre_commit_qc.clone()).await?;
                }
                Err(e) => {
    ...
    pub async fn handle_commit(&mut self, author: PublicKey, view: u64, pre_commit_qc: QuorumCert) -> ConsensusResult<()> {
        debug!("Received Commit for view {:?}", view);
        if view < self.view {
            return Ok(());
        }
